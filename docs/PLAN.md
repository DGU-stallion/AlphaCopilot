...
```
早上：打开「每日复盘」页 → 昨夜美股 / 今日 A 股开盘前信息一屏看完
上午：对话「白酒板块这 5 只与沪深300 近一年相关性怎么走的」
      → AI 写代码 → 相关性热力图 + 滚动相关曲线 → 「发布成页面」
下午：对话「用 20/60 金叉在这 5 只上回测三年，看净值和回撤」
      → 提交 job → 完成推送 → 净值/回撤双图 + 指标卡 → 存为策略草稿
晚上：对话「根据我扔进 docs 的三份年报，写一份对比报告」
