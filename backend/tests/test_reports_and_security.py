...
def test_report_roundtrip_and_delete():
    r = client.post("/api/research/myreports", json={"name": "长鑫_HBM_深度.pdf", "content_b64": _B64})
    assert r.status_code == 200
    meta = r.json()["data"]
    assert meta["industry"] == "HBM存储"
    rid = meta["id"]
    try:
        assert any(x["id"] == rid for x in client.get("/api/research/myreports").json()["data"])
        assert client.get(f"/api/research/myreports/file/{rid}").status_code == 200
    finally:
        assert client.delete(f"/api/research/myreports/{rid}").json()["data"]["ok"] is True
    assert client.get(f"/api/research/myreports/file/{rid}").status_code == 404


def test_report_illegal_type_400():
    r = client.post("/api/research/myreports", json={"name": "x.exe", "content_b64": _B64})
    assert r.status_code == 400


def test_report_data_uri_without_comma_400():
    # 之前会 IndexError→500，现应 400
    assert client.post("/api/research/myreports", json={"name": "a.pdf", "content_b64": "data:"}).status_code == 400


def test_report_missing_file_404():
    assert client.get("/api/research/myreports/file/does-not-exist").status_code == 404
...
def test_negative_cost_accepted():
    r = client.post("/api/research/portfolio/holding", json={"code": "600519", "shares": 100, "cost": -5.5})
    assert r.status_code == 200
    client.request("DELETE", "/api/research/portfolio/holding", params={"code": "600519"})  # 清理


def test_zero_shares_rejected():
    assert client.post("/api/research/portfolio/holding", json={"code": "600519", "shares": 0, "cost": 10}).status_code == 400


def test_close_bad_date_400():
    r = client.post("/api/research/portfolio/close",
