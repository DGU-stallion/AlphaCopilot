...
# SSE ticket store (single-use tokens for browser EventSource)
_SSE_TICKETS: set[str] = set()
...
_EXTRA_LOOPBACK_HOSTS: set[str] = set()  # Populated lazily via __getattr__
...
def _mint_sse_ticket() -> str:
    """Create a single-use SSE authentication ticket."""
    ticket = secrets.token_urlsafe(32)
    _SSE_TICKETS.add(ticket)
    return ticket


def _consume_sse_ticket(ticket: str) -> bool:
    """Consume a single-use SSE ticket, returning True if valid."""
    try:
        _SSE_TICKETS.remove(ticket)
        return True
    except KeyError:
        return False
...
def install_access_log_redaction_filter() -> None:
    """Install a filter on uvicorn access loggers to redact secrets from query strings."""
            ...
            if hasattr(record, "args") and isinstance(record.args, tuple):
                ...
                for i, arg in enumerate(args):
                    if isinstance(arg, str):
                        args[i] = _redact_query_secrets(arg)
                record.args = tuple(args)
            return True

    for name in ("uvicorn.access",):
        _logging.getLogger(name).addFilter(_RedactFilter())


def _redact_query_secrets(line: str) -> str:
    """Replace api_key= and ticket= values in a log line."""
    import re
    return re.sub(r"(api_key|ticket)=[^&\s]+", r"\1=***", line)
...
async def require_event_stream_auth(
    ...
    """Validate auth for browser EventSource streams (ticket-based)."""
    # Single-use ticket takes priority
    if ticket and _consume_sse_ticket(ticket):
        return
    # Reject long-lived API key in query string
    if api_key and not cred:
        raise HTTPException(status_code=401, detail="Invalid or missing API key")
    _validate_api_auth(request=request, cred=cred, query_api_key=None, allow_query=False)
...
# ============================================================================
# Lazy attribute access for module-level constants
# ============================================================================
